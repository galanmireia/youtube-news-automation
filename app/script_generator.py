...
_VARIANT_CONFIG = {
    "short": {
        "format_hint": (
            ...
            "Y ADEMAS TIENE QUE SER SIMPLE, NO SOLO UNA. Lo dijo tal cual, y no era un ejemplo "
            "suelto - \"tienes que hacer todo asi, que sean historias muy muy simples\": la "
            "caza de brujas - la gente chillando, la bruja en la hoguera - es el patron entero. "
            "Una cosa es que sea UNA sola; otra, mas dificil, es que esa una cosa sea SENCILLA "
            "de contar sin explicar nada por delante.\n\n"
            "LA PRUEBA: si para que la escena tenga gracia hace falta explicar antes que es una "
            "institucion, un cargo, un procedimiento o como funcionaba algo, esta MAL elegida - "
            "por muy UNICA que sea esa cosa. Busca la imagen fisica de dentro, la que no "
            "necesita ni una palabra de contexto:\n\n"
            "  MAL:  un tribunal religioso investigaba y sentenciaba publicamente a los "
            "acusados de herejia (hay que explicar que es un tribunal, quien lo componia, que "
            "es una herejia, como era el proceso - CUATRO cosas antes de llegar al chiste)\n"
            "  BIEN: ataban a la gente a un poste y le prendian fuego delante de todo el "
            "pueblo, que miraba y gritaba (se entiende mirando el dibujo, cero explicacion)\n\n"
            "Si el dosier es de un tema con instituciones, cargos o procedimientos por medio - "
            "tribunales, motines, cargos de gobierno -, no cuentes COMO FUNCIONABA la "
            "institucion: cuenta LA ESCENA FISICA que pasaba dentro. Esa escena es siempre mas "
            "simple que la institucion que la rodea, y es la que hay que dibujar.\n\n"
            ...
            "curiosidades cuenta COMO ERA: que comian, como se lavaban, cuanto cobraban, que "
            "hacian cuando se aburrian, que pasaba si te ponias enfermo. No hay desenlace "
            "porque no hay suceso - lo que hay es una cosa que el espectador nunca se habia "
            "parado a pensar y que resulta ser rarisima.\n\n"
            "Cada escena tiene UN TRABAJO:\n\n"
            "  1. EL DATO QUE NO TE CREES. Sueltalo pelado, sin fecha ni sitio, como quien "
            "comenta algo en una cena. Tiene que dar ganas de decir '¿que?'.\n"
            "  2. COMO ERA DE VERDAD. Ahora si, el cuando y el donde, pero dentro de lo "
            "concreto: quien lo hacia, con que, cada cuanto. Nada de contexto historico.\n"
            "  3. EL DETALLE QUE LO REMATA. El segundo dato, mas raro que el primero. Aqui es "
            "donde se queda o se va: si la escena 3 no añade nada nuevo, el video se acabo.\n"
            "  4. Y ENCIMA, POR QUE LO HACIAN. El motivo, que casi siempre es mas absurdo que "
            "la propia costumbre: por dinero, por miedo, por una ley mal escrita, porque "
            "siempre se habia hecho asi.\n"
            "  5. EL REMATE. Lo que quedo de aquello, o el dato final que deja al espectador "
            "con la cara torcida. NO es una moraleja, NO es un 'suscribete' y NO es una "
            "reflexion sobre el paso del tiempo. Si suena a final de documental de La 2, "
            "esta mal.\n\n"
            "LA PRUEBA: si al terminar el espectador no puede contarselo a alguien en una "
            ...
            "OTRA FORMA, PARA CUANDO LA HISTORIA ES UNA DISCUSION QUE SE LES VA DE LAS MANOS "
            "(un motin, una guerra que empieza por una tonteria, una rebelion): no la cuentes "
            "como curiosidad de 5 pasos, cuentala como una DISCUSION QUE ESCALA, casi todo en "
            "bocadillos, casi nada narrado. Ella misma trajo el ejemplo entero - la Guerra de "
            "la Oreja de Jenkins -, y esto es como se traduce a este canal:\n\n"
            "  Escena 1 (calle): el ingles acusa. «¡ME HAS CORTADO LA OREJA!» «¿ESA ES TU "
            "OREJA?» «SI» «PUES ENHORABUENA» «¡NO ES PARA FELICITARME!»\n"
            "  Escena 2 (salon_trono): se lo cuenta al rey. «¡MI OREJA, MAJESTAD!» «ESTO NO "
            "PUEDE QUEDAR ASI... ¡¡¡GUERRA!!!»\n"
            "  Escena 3 (calle): España no se lo cree. «¿POR UNA OREJA?» «¡POR UNA OREJA!»\n"
            "  Escena 4 (campo o cubierta): el ejercito, con banderas de los dos paises - "
            "\"bandera_espana\" y \"bandera_inglaterra\" en la misma escena, que es lo que "
            "explica el conflicto sin decir nada mas. NO hacen falta veinte monigotes: DOS o "
            "TRES con pose \"peleando\" o \"corriendo\" y sonido \"gentio\" leen igual de "
            "bien como \"un ejercito entero\" que veinte figuras que no caben en el plano.\n"
            "  Escena 5, EL REMATE: la pregunta que desinfla toda la guerra. «¿Y LA OREJA "
            "ESTA BIEN?» «...No.» Silencio. Esto es EL REMATE de siempre (paso 5 de arriba), "
            "solo que en dialogo en vez de narrado: la escena final vuelve al detalle "
            "minusculo que empezo todo, no a una moraleja.\n\n"
            "Cada intercambio es una escena CORTA y un CORTE nuevo, nunca una escena larga con "
            "diez frases seguidas: son las mismas MAXIMO DOS citas por escena de siempre, solo "
            "que encadenando muchas escenas rapidas en vez de pocas largas. Y siempre empieza "
            "YA DENTRO de la discusion - nunca \"habia una vez\" ni situando antes: la primera "
            "frase de la escena 1 ya es alguien gritando algo.\n\n"
